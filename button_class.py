...
class Button:
    ...
    def draw(self, win, outline=True):
        ...
        if outline:
            pygame.draw.rect(win, self.active_colour, (self.x - 2, self.y - 2, self.width + 4, self.height + 4), 0, 10)

        self.buttonRect = pygame.draw.rect(win, self.colour, (self.x, self.y, self.width, self.height), 0, 8)

        if self.buttonRect.collidepoint(pygame.mouse.get_pos()) or self.active:
            self.active_colour = self.given_bd_colour
        else:
            self.active_colour = (12, 15, 26)


        if self.buttonRect.collidepoint(pygame.mouse.get_pos()) or self.active or self.highlight_btn:
            self.active_colour = self.given_bd_colour
        else:
            self.active_colour = (12, 15, 26)
...
class Dropdown(Button):
    ...
    def draw(self, surf):
        ...
        pygame.draw.rect(surf, self.active_colour if self.show_options else self.colour, self.rect, 1, 19) # If hovered over
        ...
        if self.draw_menu:
            for i, text in enumerate(self.option_list):
                ...
                pygame.draw.rect(surf, self.active_colour if i == self.active_option else self.colour, rect, 0)
