...
import random
from validation import validate_user_details, username_exists
...
def request_page(frame, w):
    ...
    for x in range(0, count_requests()): # iterate for every request in REQUESTS table
        ...
        Label(frame, text=f"{request_lst[x][0]: <50}{request_lst[x][1]: <50}",
              font=("Gotham", 12), fg="#0a95ad", bg=my_grey).place(x=20, y=y)
...
def overview_page(frame):
    ...
    for i in range(3):
        ...
        Label(frame, text=labels[i], font=("Gotham", 12), fg="white", bg=my_grey)\
...
# # #
# # # insert_user('PyAdmin727', 'MyAdmin328782')
